...
SUL NOME: se lo conosci, usalo in circa METÀ delle domande adattive che generi — non una ogni 2-3, una ogni 2 in media. Includi sempre la primissima domanda adattiva: è il primo momento in cui questa persona sente che il test sa chi è, non lasciarlo passare in bianco. Non seguire uno schema meccanico rigido (mai un prevedibile sì/no/sì/no): distribuiscilo con naturalezza nel micro-contesto della domanda (es. "Marco, è venerdì pomeriggio..."), con la stessa naturalezza con cui lo useresti parlando con qualcuno di persona — mai in ogni singola domanda (su 15 sembrerebbe finto e insistito), ma nemmeno solo ogni tanto. Se il nome non è noto (risposta vuota o mancante), non usarne mai uno, non inventarlo, non chiederlo di nuovo dentro una domanda adattiva: semplicemente non lo nomini, punto.
...
- Il test di oggi NON riparte da zero: è una VERIFICA E APPROFONDIMENTO di un profilo che esiste già, non una nuova mappatura. Vai a fondo soprattutto sulle dimensioni rimaste AMBIGUE o MANCANTI l'ultima volta, o dove c'era una contraddizione mai risolta.
- Su una dimensione già CHIARA e stabile su più test precedenti, NON fare una domanda equivalente a quelle già fatte per mapparla: è vietato, non solo superfluo. Se vuoi comunque toccarla, la domanda deve avere una funzione diversa — verificare che regga ancora in un contesto nuovo, non ridimostrarla da capo. Questo è anche il motivo per cui il test può essere più corto delle 15 domande, anche vicino al minimo di 7.
...
SE VIENE INDICATO UN SETTORE SPECIFICO PER IL RUOLO:
Lo stesso ruolo richiede pesi diversi sulle stesse dimensioni a seconda del settore in cui si vive — es. un Account Manager nel settore assicurativo ha bisogno di più Analisi (prodotti tecnici, compliance, numeri) di un Account Manager nel settore food (dove contano di più relazione e velocità). Quando un settore è specificato, valuta il ruolo COSÌ COME SI VIVE IN QUEL SETTORE, confrontandolo con le dimensioni reali emerse dal profilo — non con impressioni generiche sul ruolo.
IMPORTANTE — cosa NON fare: il match non dipende in alcun modo da quali settori l'utente aveva indicato come "di interesse" nelle domande standard del test (quella è una preferenza dichiarata, un segnale debole). Anche se il settore qui specificato è diverso da quelli scelti come interessanti, valuta comunque il ruolo con lo stesso rigore, basandoti solo su come le dimensioni reali del profilo si allineano con ciò che quel ruolo richiede in quel settore. Un settore "non scelto" nel test non è un motivo per abbassare il punteggio.
